...
: ydrop  yst @ @ cs cells cell+ negate yst +! ;

cell 1- log~ constant cellshift

: @depth ( ad -- n )  dup @ swap - cellshift rshift ;
: @cl ( ad -- )  dup ! ;
: @empty ( ad -- flag )  dup @ = ;
\ example: xst @empty
\ _depth _cl _empty _fence
\ @depth @cl @empty @fence
...
: showz zst @depth if zst> >r recurse r> dup . >zst then ;
: showy yst @depth if yst> >r recurse r> dup . >yst then ;
...
: set-member \ -- flag | s s' -- 
  ...
  while zst @split zst@ ?obj 2 =      \ element is a set?
     if yst zst setcopy set=  
...
: reduce \ -- | s -- s'   ??? recursivt!!!
  0 >yst foreach
  ?do zst @fence zdup zst> drop
     yst zst setcopy member
     if zdrop
     else yst zst setmove
        merge zst yst setmove
     then
...
: intersection \ -- | s s' -- s" 
  ...
  begin zst@
  while zst @split zst @fence zdup zst> drop
     yst zst setcopy member
     if xst zst setmove merge zst xst setmove
     else zdrop
     then 
...
: diff \ s s' -- s" 
  ...
  while zst @split zst @fence zdup zst> drop
     yst zst setcopy member
     if zdrop 
     else xst zst setmove merge zst xst setmove
     then
  repeat zdrop yst setdrop
  xst zst setmove reduce ;
...
: multincl \ -- | s x -- s'    ???
  0 >xst zst @fence zst yst setmove 
  begin zst@ 
  while zst @split yst zst setcopy union zst @fence 
...
: powerset \ s -- s' 
  ...
  zst @split zst @fence zst yst setmove recurse 
  zdup yst zst setmove zst> drop multincl union ; 

: cartprod \ s s' -- s"
  zst yst setmove
  zst xst setcopy xst> drop card 0 0 >zst
  ?do xst @fence -1 xst+! 
     yst setdup
     begin yst@
     while yst @split yst @fence -1 yst+!
        xst zst setcopy
...
: power# \ n -- | s -- s' 
  dup zdup card = if drop zst @fence exit then 
  dup 1 = if drop infence exit then 
  zst @split zst @fence zst xst setmove 
  dup zdup recurse 
...
: setcup \ -- | s s' -- s"
  ...
  ?do xst zst setcopy union zst @fence
...
: setcap \ -- | s s' -- s" 
  zst xst setmove 0 >yst foreach 
  ?do xst zst setcopy intersection zst @fence 
  yst zst setmove union zst yst setmove 
  loop xst setdrop yst zst setmove ; 
...
: q  xst @cl yst @cl zst @cl 0 match ! abort ; 
